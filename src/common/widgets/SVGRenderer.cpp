...
namespace Compose
{
  void SVGRenderer::setModifier(const SVGFileContent& svgContent) const
  {
    ...
    if(document)
    {
      ...
      auto& svgData = widget.ensureDataForKeyExistsOwning<SVGData>(c_svgData, [cb = [](DrawContext&, int, int) {}] { return new SVGData(cb); });

      svgData.document.modify([&document](auto& doc) { doc = std::move(document); });

      setDrawCall([handle = getHandle()](DrawContext& ctx, int width, int height) {
        const Widget widget(handle);
        const auto& svgData = widget.ensureDataForKeyExistsOwning<SVGData>(c_svgData, [cb = [](DrawContext&, int, int) {}] { return new SVGData(cb); });
  ...
  {
    auto& svgData = ensureDataForKeyExistsOwning<SVGData>(c_svgData, [cb = [](DrawContext&, int, int) {}] { return new SVGData(cb); });
    svgData.color = col;
  }
}
