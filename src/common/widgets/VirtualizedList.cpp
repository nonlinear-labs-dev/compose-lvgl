...
namespace Compose
{
  ...
  struct VirtualizedList::State
  {
    ...
    void ensureHandlers()
    {
      assert(handle && lv_obj_is_valid(handle));
    ...
    {
      const auto viewportExtent = axis == Axis::Vertical ? lv_obj_get_content_height(targetHandle) : lv_obj_get_content_width(targetHandle);
    ...
    {
      const auto rowGap = axis == Axis::Vertical ? lv_obj_get_style_pad_row(targetHandle, LV_PART_MAIN) : lv_obj_get_style_pad_column(targetHandle, LV_PART_MAIN);
    ...
    {
      const auto spacersValid = topSpacer && bottomSpacer && lv_obj_is_valid(topSpacer) && lv_obj_is_valid(bottomSpacer) && lv_obj_get_parent(topSpacer) == listHandle
          && lv_obj_get_parent(bottomSpacer) == listHandle;

      const auto rowsValid = static_cast<int>(rows.size()) == renderedItems
          && std::all_of(rows.begin(), rows.end(), [listHandle](const auto row) { return row && lv_obj_is_valid(row) && lv_obj_get_parent(row) == listHandle; });
    ...
    [[nodiscard]] lv_obj_t *currentValidHandle() const
    {
      ...
      assert(listHandle && lv_obj_is_valid(listHandle));
      return listHandle;
    }
