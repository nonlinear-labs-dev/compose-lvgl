...
namespace Compose
{
  ...
  static OrderedJson resolveAssignedValue(const OrderedJson& value, const ValueAssignments& assignments,
  ...
  {
    j = nlohmann::json { { "size", in.size },
                         { "weight", Font::getWeightString(in.weight) },
                         { "baseName", in.baseName },
                         { "fallbackBaseNames", in.fallbackBaseNames } };
  }
...
{
  template <typename T>
  void addProperty(nlohmann::json& json, const char* key, const std::optional<T>& value, bool onlyNonNullOptProperties)
  {
    if(value)
  ...
  std::string Style::dump(bool onlyNonNullOptProperties) const
  {
    ...
    addProperty(ret, "background-color", std::get<std::optional<BackgroundColor>>(properties),
                onlyNonNullOptProperties);
  ...
  using JsonModifier = std::variant<BackgroundColor, PrimaryColor, Font, TextAlign, VerticalAlign, FlexAlign, Width,
                                    Height, Margin, MarginLeft, MarginRight, MarginTop, MarginBottom, Padding, Border,
                                    BorderWidth, BorderColor, BorderSides, RoundedCorner, Scrollable>;
  ...
  static StyleSheet parseStyleSheet(const std::string& name, const OrderedJson& definition,
                                    const ValueAssignments& inheritedAssignments = {},
    ...
    for(const auto& [key, value] : definition.items())
    {
      if(isMergedClassName(key))
      {
        ...
        if(mergedClassName.empty())
          ...
          sameContextSheets->emplace_back(
              parseStyleSheet(name + "-" + mergedClassName, value, assignments, sameContextSheets));
        else
