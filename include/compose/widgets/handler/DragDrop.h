...
namespace Compose
{
  ...
  class DragDropContext
  {
   public:
    ...
    void setSource(lv_obj_t *self, const std::string &type, int offsetX, int offsetY, int rootX, int rootY,
                   const Getter &getter, const DragWidgetBuilder &dragWidgetBuilder);
   ...
   private:
    ...
    struct Source
    {
      Source(lv_obj_t *widget, const std::string &type, int offsetX, int offsetY, int rootX, int rootY,
             const Getter &getter, const DragWidgetBuilder &dragWidgetBuilder);
      ~Source();
  ...
  struct DragDrop
  {
    ...
    struct DragDropForContent
    {
      ...
      struct Source
      {
        ...
        struct Data
        {
          ...
          Data(lv_obj_t *handle, std::string type, const Getter &getter,
    ...
    DragDropForContent &operator()(const std::string &type,
                                   DragDropForContent::Source::StartAxis startAxis
                                   = DragDropForContent::Source::StartAxis::ANY());
...
#define DRAG_DROP_VERTICAL(type)                                                                                       \
  it.dragDrop(type, Compose::DragDrop::DragDropForContent::Source::StartAxis::VERTICAL())                              \
      << [=](Compose::DragDrop::DragDropForContent * it)
